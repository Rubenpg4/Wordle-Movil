...
flowchart TB
    subgraph UI["🎨 Capa de Presentación"]
        WA[WordleApp] --> PI[PaginaInicio]
        PI --> PJ[PaginaJuego]
        PI --> RK[Ranking]
    end
    
    subgraph Components["🧩 Componentes Reutilizables"]
        CJ[ContainerJuego<br/>Casilla del tablero]
        CL[ContainerLetra<br/>Tecla del teclado]
    end
    
    subgraph Logic["⚙️ Lógica de Negocio"]
        TI[TipoIdioma<br/>Enum con rutas]
        TCJ[TipoCampoJuego<br/>Estados visuales]
        TCL[TipoCampoLetra<br/>Estados teclado]
        TB[TipoBorde<br/>Activo/Inactivo]
        TP[tipoPista<br/>Bomba/Lupa]
    end
    
    subgraph Data["💾 Datos Persistentes"]
        CF[config.txt<br/>Preferencias]
        RF[Ranking.txt<br/>Top 10 jugadores]
    end
    
    subgraph Assets["📦 Assets"]
        DIC[Diccionarios<br/>~9.6M palabras]
        IDI[Traducciones]
        ICO[Iconos]
    end
